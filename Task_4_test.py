...
def main():
    checkpoint = torch.load("saved_model/model_4.pth", map_location=DEVICE)
    with open("saved_model/hyper_4.json", "r") as f:
    ...
    model_gnn.eval()
    model_mlp.eval()
    test_csv = "data/dataset_index/Task_4_test.csv"
    ...
    with open("results/task_4.txt", "w") as f:
        f.write(result_str)
    print("=== 测试完成，结果已保存到 results/task_4.txt ===")
